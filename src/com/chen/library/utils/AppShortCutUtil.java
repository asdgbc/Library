...
public class AppShortCutUtil {
	...
	public static boolean isAddShortCut(Context context) {
		...
		/*
		 * int versionLevel = android.os.Build.VERSION.SDK_INT; String AUTHORITY = "com.android.launcher2.settings"; //2.2以上的系统的文件文件名字是不一样的 if (versionLevel >= 8) { AUTHORITY =
		 * "com.android.launcher2.settings"; } else { AUTHORITY = "com.android.launcher.settings"; }
		 */
	...
	public static void xiaoMiShortCut(Context context, Class<?> clazz, String num) {
		...
		try {
			...
			builder.setContentTitle("您有" + num + "未读消息").setContentText("您有" + num + "未读消息").setSmallIcon(R.drawable.ic_launcher);
			notification = builder.build();
			Field field = notification.getClass().getDeclaredField("extraNotification");
			Object extraNotification = field.get(notification);
			Method method = extraNotification.getClass().getDeclaredMethod("setMessageCount", int.class);
			method.invoke(extraNotification, Integer.valueOf(num));
		} catch (Exception e) {
			// e.printStackTrace();
			// // miui 6之前的版本
			isMiUIV6 = false;
